...
const MOBILE_POSITIONS = {
  hero: { x: '0vw', y: '-5vh', scale: 0.88, rotation: 0, opacity: 1, blur: 0, z: 20 },
  next: { x: '32vw', y: '0vh', scale: 0.34, rotation: 7, opacity: 0.62, blur: 2, z: 8 },
  prev: { x: '-32vw', y: '0vh', scale: 0.34, rotation: -7, opacity: 0.62, blur: 2, z: 8 },
  hiddenRight: { x: '62vw', y: '4vh', scale: 0.22, rotation: 13, opacity: 0, blur: 10, z: 2 },
  hiddenLeft: { x: '-62vw', y: '4vh', scale: 0.22, rotation: -13, opacity: 0, blur: 10, z: 2 },
}

const wrap = (n) => (n + PRODUCTS.length) % PRODUCTS.length
const getPositions = () => typeof window !== 'undefined' && window.innerWidth <= 768 ? MOBILE_POSITIONS : POSITIONS
...
export default function SpiritCarousel() {
  ...
  const dragRef = useRef({ id: null, x: 0, y: 0, active: false, dragged: false })
  ...
  function onPointerMove(event) {
    if (!stageRef.current || lockedRef.current || isCoarsePointer()) return
  ...
  function onPointerDown(event) {
    if (lockedRef.current || detailOpen) return
    dragRef.current = { id: event.pointerId, x: event.clientX, y: event.clientY, active: true, dragged: false }
  ...
  function onPointerUp(event) {
    ...
    if (Math.abs(dx) > 14 && Math.abs(dx) > Math.abs(dy) * 1.15) gesture.dragged = true
    if (Math.abs(dx) > 55 && Math.abs(dx) > Math.abs(dy) * 1.15) changeProduct(dx < 0 ? 1 : -1)
  }

  function openDetail() {
    if (lockedRef.current || detailOpen || dragRef.current.dragged) { dragRef.current.dragged = false; return }
    setDetailOpen(true)
    requestAnimationFrame(() => {
      ...
      gsap.timeline({ defaults: { ease: 'power4.inOut' } })
        ...
        .to(hero, { '--x': isCoarsePointer() ? '-3vw' : '-22vw', '--y': isCoarsePointer() ? '-17vh' : '0vh', '--scale': isCoarsePointer() ? 0.74 : 1.08, '--rotation': '0deg', filter: 'brightness(1.2) drop-shadow(0 28px 22px rgba(0,0,0,.42))', duration: 0.85 }, 0.08)
  ...
  function closeDetail() {
    ...
    gsap.timeline({ defaults: { ease: 'power4.inOut' }, onComplete: () => { setDetailOpen(false); resetParallax() } })
      ...
      .to(hero, { '--x': positions.hero.x, '--y': positions.hero.y, '--scale': positions.hero.scale, filter: 'drop-shadow(0 28px 22px rgba(0,0,0,.36))', duration: 0.8 }, 0.05)
  ...
  return (
    <>
      <style>{`\n        .nts-spirit-stage { position: relative; }\n        .nts-detail-close { transition: transform .2s ease, background .2s ease; }\n        @media (max-width: 768px) {\n          .nts-spirit-stage { min-height: calc(100svh - 76px) !important; height: calc(100svh - 76px); touch-action: pan-y; }\n          .nts-spirit-bottle { width: 64vw !important; max-width: 72vw !important; max-height: 55svh !important; }\n          .nts-spirit-label { top: 20px !important; left: 20px !important; }\n          .nts-spirit-ghost { font-size: 19vw !important; line-height: .82 !important; max-width: 100vw; overflow: hidden; }\n          .nts-spirit-info { left: 20px !important; bottom: 102px !important; width: calc(100vw - 40px) !important; max-width: none !important; }\n          .nts-spirit-info .nts-description { font-size: 11px !important; line-height: 1.45 !important; max-width: 330px !important; margin-top: 9px !important; }\n          .nts-spirit-controls { left: 20px !important; bottom: 20px !important; gap: 8px !important; }\n          .nts-spirit-controls button { width: 42px !important; height: 42px !important; }\n          .nts-spirit-action { right: 20px !important; bottom: 27px !important; font-size: 20px !important; gap: 5px !important; }\n          .nts-spirit-action svg { width: 20px !important; height: 20px !important; }\n          .nts-detail { padding: 78px 20px 60px !important; }\n          .nts-detail-close { top: 16px !important; right: 16px !important; width: 42px !important; height: 42px !important; }\n          .nts-detail-info { margin-left: 0 !important; max-width: none !important; padding-top: 43svh !important; padding-bottom: 40px !important; }\n          .nts-detail-info h2 { font-size: clamp(48px, 18vw, 78px) !important; }\n          .nts-detail-info > p { font-size: 19px !important; }\n          .nts-detail-stats { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; gap: 16px !important; margin: 34px 0 !important; padding: 18px 0 !important; }\n          .nts-detail-stats > div:nth-child(3) { grid-column: 1 / -1; }\n          .nts-detail-section h3 { font-size: clamp(28px, 9vw, 44px) !important; }\n          .nts-detail-end { min-height: 24vh !important; margin-top: 42px !important; }\n        }\n        @media (max-width: 380px) {\n          .nts-spirit-info { bottom: 94px !important; }\n          .nts-spirit-info h2 { font-size: 28px !important; }\n          .nts-spirit-info .nts-product-name { font-size: 13px !important; margin-top: 9px !important; }\n          .nts-spirit-action { font-size: 18px !important; }\n          .nts-detail-info { padding-top: 40svh !important; }\n        }\n        @media (prefers-reduced-motion: reduce) {\n          .nts-spirit-stage * { scroll-behavior: auto !important; }\n        }\n      `}</style>
      <section className="nts-spirit-stage" ref={stageRef} onPointerMove={onPointerMove} onPointerLeave={() => resetParallax(true)} onPointerDown={onPointerDown} onPointerUp={onPointerUp} style={{ position: 'relative', minHeight: 'calc(100vh - 110px)', overflow: 'hidden', isolation: 'isolate', color: '#fff', background: 'radial-gradient(circle at var(--glowX,50%) var(--glowY,46%), color-mix(in srgb, var(--tone3,#555) 45%, transparent), transparent 28%), linear-gradient(135deg, var(--tone1,#111), var(--tone2,#222) 55%, #050505)', transition: 'background 0.25s ease', touchAction: 'pan-y' }}>
